...
class App extends Controller
{
    ...
    public function home(): void
    {
		...
		$events =(new Maintenance())->find("application_id= :aplication","aplication={$this->user->application_id}");
		
        echo $this->view->render("home", [
            "head" => $head,
             "events" => $events->order("date_initial")->fetch(true)
        ]);
    }
    ...
    public function supplier(?array $data): void
    {
        ...
        $pager->pager($suppliers->count(),$suppliers->count(), (!empty($data["page"]) ? $data["page"] : 10));
        ...
        if (!empty($data["action"]) && $data["action"] == "delete") {
            ...
            if (!$supplierDelete) {
                ...
                echo json_encode(["redirect" => url("/app/supplier")]);
                return;
        ...
        echo $this->view->render("supplier", [
            "head" => $head,
			"search" => $search,
            "suppliers" => $suppliers->order("name desc")->limit($pager->limit())->offset($pager->offset())->fetch(true),
    ...
    public function supplierAdd(?array $data): void
    {
        ...
        if (!empty($data["action"]) && $data["action"] == "create") {
			...
			$supplierCreate->occupation_id = $data["occupation_id"];
